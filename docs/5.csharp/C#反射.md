...
| FieldInfo | 该类的字段 | GetFields/GetField |
...
| NonPublic | 类中私有成员,根据oop原则基类私有成员，被继承就不可访问.所以只会返回基类protected(继承后为private)与internal 成员 |
| FlattenHierarchy | 继承层次结构的属性 |
| DeclaredOnly | 仅搜索该Type表示的类中的属性，基类不搜索 |
...
:::info
.net7 之后返回的属性根据元数据排序，返回的属性顺序是确定的.
:::
...
:::tip
不管是CreateInstance还是CreateInstanceFrom都需要加载对应程序集才能创建其中类型的实例
:::
...
:::tip
关于 CultureInfo 参数默认是当前类型的CultureInfo。不同区域,表示1000的字符串不同转化double自然不同。
:::
...
提供了两个属性(这里回去要看下委托)
- AddMethod = GetAddMethod 值为 true(返回非公共方法)
- RemoveMethod = GetRemoveMethod 值为 true
