...
常见实践是在 LINQ 中向 `System.Collections.Generic.IEnumerable<T>` 添加方法。

见 MSDN 内容：如何实现和调用自定义扩展方法
...
::: tip 注意
重载解析首选类型本身定义的实例或静态方法，而不是扩展方法。扩展方法没有被扩展类任何私有类的访问权限。
:::
...
这里 val 变量在声明处到 lambda 表达式结束作用域都可见，相当于：
...
函数传递参数，参数传递本身是值传递的，不管引用类型还是值类型。引用类型只是传递了"引用（指针）"的副本，虽然都指向同一个对象。

加了 `ref` 修饰后传递的就是引用本身了，此时函数内部更换对象会影响到外部。
