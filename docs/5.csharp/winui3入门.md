...
### xmal
这是一种类似于html的标记型语言
来源于xml,xml原意是用来传输数据的，经过改造的xmal用来生成ui布局的设计文件
和html不同的是 html可以通过浏览器映射到DOM元素 通过js可以动态删除修改DOM元素节点

而xaml则是声明式标记语言,他被编译后生成对应的c++/c# 代码 的ui文件*.g.cs

winui3项目(笼统的说) 通过xaml编译器将xmal编译成对应的c++/c#代码 将xaml声明的控件元素映射到.cs的类中去
...
使用
Microsoft.UI.Xaml.Controls 命令控件的控件
需要这样引入
...
TextBox、PasswordBox、RichEditBox、ComboBox、Slider、ToggleSwitch、CheckBox、RadioButton
...
ListView、GridView、ItemsRepeater、DataGrid（社区版）
...
NavigationView、Frame、Pivot（旧）/ TabView
...
ContentDialog、MessageDialog、TeachingTip、ToolTip
...
ProgressBar、ProgressRing、RatingControl
...
## 绑定数据
x:Bind 和Binding
ElementName或 RelativeSource 引入属性
