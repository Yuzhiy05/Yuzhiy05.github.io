...
为了减少泛型造成的代码膨胀，CLR 会只编译一次泛型类的方法（待验证）。

`List<String>` 和 `List<Stream>` 中的编译的方法能共享，因为引用类型的变量都是指向堆上的 32/64 位的指针。

值类型大小不一，泛型生成的方法不能公用。即使大小一样 `Int32` 和 `UInt32` 也不能共用，因为生成的指令不一样。
...
// 这个模板参数能接受的类型，不限于 T 具体有哪些继承？需要讨论
```
